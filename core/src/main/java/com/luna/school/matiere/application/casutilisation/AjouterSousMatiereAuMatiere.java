...
public class AjouterSousMatiereAuMatiere {
  ...
  public void ajouterSousMatiere(AssocierSousMatiereAuMatiereCommande commande) {
    ...
    this.verifierLibelleParMatiere(sousMatiere.getNomSousMatiere(), matiere.getId());
  ...
  private void verifierLibelleParMatiere(String libelle, UUID MatiereId) {
    boolean existeParLibelle = this.matiereRepositoryPort
        .existsParLibelleParMatiere(libelle, MatiereId);
    if (existeParLibelle) {
      throw new MatiereException(
          "Le libelle " + libelle + "existe déjà pour cette matier !");
    }
  }
}
